...
| **`spawn_codex_sync`** | LLM 分析主路径：严格 prompt + 10min 超时，自带并发 permit / 超时 SIGKILL / 错误归类 | [analyze/main.lua:40](packages/audit-analyzer/departments/analyze/main.lua) |
...
| **`raise(queue, payload)`** | payload 只带指针（batch_id/dedup_key/source_ref），大内容落 cache | [collect/main.lua:77](packages/audit-watcher/departments/collect/main.lua) |
...
| fkst 设计模式 | 本仓库的落地 |
|---|---|
| **外部系统即数据库**（包内无持久业务态） | 文件即事实源：offset/批内容/分析结果/去重标记全是 scratch cache，可丢；丢了就全量重读，重复被幂等层吸收（[collect/main.lua:262-266](packages/audit-watcher/departments/collect/main.lua)） |
...
| **payload 小指针化** | 大批内容落 `cache_set(batch_content_key…)`，事件只带 `batch_id`；analyzer 回源取（[collect/main.lua:75](packages/audit-watcher/departments/collect/main.lua)、[analyze/main.lua:67](packages/audit-analyzer/departments/analyze/main.lua)） |
...
| **预算处处有界** | 批 ≤24KB、单行 ≤2KB、每轮最多 5 个 finding、Aevatar 默认每 tick 2 页且配置硬上限 50 页、retry 有上限——没有无界循环（[audit-watcher/core.lua](packages/audit-watcher/core.lua)、[audit-analyzer/core.lua](packages/audit-analyzer/core.lua)） |
...
- **源 B（Aevatar 云审计日志流）**：
  ...
  - **读取方式**：在 [collect/main.lua](file:///Users/eanzhao/Code/fkst-audit-log/packages/audit-watcher/departments/collect/main.lua) 中，通过执行本地 NyxID 工具发送代理请求：`nyxid proxy request aevatar /api/audit/trail`。拉取时默认按当前时间往回切 10 分钟时间片滚动扫描（防止接口升序分页卡在窗口开头），对已拉取的每条 audit id 进行去重（基于 seen-id 缓存，7 天 TTL）。
...
- **反幻觉 fail-closed**：LLM 输出必须是严格密集 JSON 数组、逐字段限长、severity 合法、条数 ≤5，任何不合格一律 `error()` 走 DLQ；证据行必须逐字出现在原文（[audit-analyzer/core.lua:88-137](packages/audit-analyzer/core.lua)）。
...
2. **反幻觉门禁**（[core.lua:evidence_present](packages/audit-analyzer/core.lua#L145-L147)）：验证 Finding 中的 `evidence_line` 是否真实且逐字存在于投喂给模型的原始 `batch_lines` 文本中。若不匹配，则判定模型产生了幻觉（Fabricated Evidence），该 Finding 会被丢弃不触发告警。
