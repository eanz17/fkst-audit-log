...
[audit-watcher.collect]  consumes: audit_file_changed, audit_sweep_tick
   读文件增量 → 预过滤/分批 → 大内容落 cache，raise 小指针事件
...
[audit-analyzer.analyze]  consumes: audit-watcher.audit_batch（跨包消费用限定名，
   ...
   回源取日志片段 → spawn_codex_sync（或 exec_sync+curl 直连 LLM API）
...
local M = {}
M.spec = {
  consumes = { "audit_file_changed", "audit_sweep_tick" },
  produces = { "audit_batch" },            -- 自有队列；analyzer 用 audit-watcher.audit_batch 消费
  stall_window = "2m",
  retry = { max_attempts = 5, base = "10s", cap = "5m" },
}

local MAX_BATCH_BYTES = 32 * 1024          -- payload 有 64KiB 上界，留余量

function pipeline(event)
  local path = (event.payload or {}).path
  if not path then return end               -- sweep tick：可在此扫描 glob 做兜底
  local key = "audit/offset/" .. path:gsub("/", "_")
  with_lock(key, function()
    local offset = tonumber(cache_get(key)) or 0
    local content = file.read(path)
    if #content < offset then offset = 0 end          -- 文件被轮转，从头读
    local chunk = content:sub(offset + 1)
    if chunk == "" then return end
    -- 预过滤：只把命中关键词/严重级别的行送 LLM（降噪省钱的第一道闸）
    local suspicious = {}
    for line in chunk:gmatch("[^\n]+") do
      if line:match("DENIED") or line:match("sudo") or line:match("failed")
         or line:match("privilege") then
        table.insert(suspicious, line)
      end
    end
    cache_set(key, tostring(#content))
    if #suspicious == 0 then return end
    -- 大内容落 cache，事件只带指针（学 consensus 的 content_fetch 模式）
    local batch_id = path:gsub("/", "_") .. "-" .. tostring(now())
    cache_set("audit/batch/" .. batch_id, table.concat(suspicious, "\n"), 3600)
    raise("audit_batch", {
      schema = "audit-watcher.batch.v1",
      batch_id = batch_id,
      source_path = path,
      line_count = #suspicious,
      dedup_key = "audit-batch/" .. batch_id,
      source_ref = { kind = "external", ref = path .. "#" .. tostring(#content) },
    })
  end)
end
return M
...
function M.build_prompt(log_lines, max_findings)
  return table.concat({
    ...
    'Schema: {"severity":"critical|high|medium","category":"...","evidence_line":"<原文行>",'
...
function pipeline(event)
  ...
  local lines = cache_get("audit/batch/" .. tostring(p.batch_id))
  if not lines then
    log.warn("audit-analyzer: stale batch, cache evicted; skip " .. tostring(p.batch_id))
    return   -- 旧世代重投：静默放弃（学 consensus 的 stale_generation 处理）
  end
  local result = spawn_codex_sync({
    prompt = core.build_prompt(lines, 5),
    timeout = 600,
  })
  ...
  for _, f in ipairs(findings) do
    -- 反幻觉核验：evidence_line 必须真实出现在原始批次里（学 archaudit 的 git show 核验）
    if lines:find(f.evidence_line, 1, true) and f.severity ~= "medium" then
      raise("alert-proxy.alert_request", {
        ...
        dedup_key = core.alert_dedup_key(f),   -- 内容派生：category+evidence 摘要+时间桶
      })
    end
  end
end
...
- **路 A（引擎正统路径）：`spawn_codex_sync`**。自带并发 permit 池、超时 SIGKILL、日志留痕、`error_class` 归类（quota/throttle/auth/unavailable）、崩溃收养。约束是模型钉死在 host 安装的 codex CLI 上。
- **路 B（任意 LLM API）：`exec_sync` + curl**。引擎无 HTTP 原语，这是唯一直连方式，合法且被审计（每次调用留 `EVENT=external_command` 日志）。注意三个坑：`json` 没有 encode（请求体用 `file.write` 写临时文件 + `curl -d @file`）；stdin 恒为 /dev/null（不能 `-d @-`）；限流自己配 `FKST_RATE_POOL_CURL=<burst>,<refill/min>`，拿不到 token 会阻塞，要和 `stall_window` 匹配。**日志含敏感信息时优先本地模型**（Ollama/vLLM 的 OpenAI 兼容端点），或在 collect 阶段先脱敏（借鉴 K8sGPT 的替换-还原方案）。
...
| LLM 幻觉误报 | evidence 行事实核验（不在原文中即丢弃）+ severity 阈值 + 高危告警可加"第二个角度复核"（照抄 consensus 包的多角度共识，一票否决制） |
| 敏感日志外泄 | 本地模型（Ollama/vLLM）或送出前脱敏；exec_sync 不清洗任何 env，webhook URL/API key 放 host `fkst.env`，不进代码 |
| 告警风暴 | 内容派生 dedup_key + 24h cache 窗口 + `FKST_RATE_POOL_CURL` 限流 |
